 ...
 */
public class IndictorView extends View implements ViewPager.OnPageChangeListener {

    //指示器图片  这里是一个Drawable 包含两种状态
//    选中和为选中
    private Drawable mIndicator;
    private Drawable redIndicator;
    ...
    //   图标加空格在加上 padding的 宽度
    private int mContextWidth;

    //    指示器图标的个数  就是当前Viewpager item 个数
    private int mCount;

    //    每个指示器之间的间隔大小
    private int mMargin;

    //    当前view 的item 主要作用,用于判断当前指示器的选中情况
    ...
    //    因为viewpager 的pageChangelistener 被占用了  所以需要定义一个以便其他调用
    ...
    public IndictorView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.redIndicator = getResources().getDrawable(R.drawable.red_yuanlian);

//        通过typedarray获取自定义属性
        TypedArray typedArray = getResources().obtainAttributes(attrs, R.styleable.IndictorView);
//   获取自定义属性的个数
        int N = typedArray.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = typedArray.getIndex(i);
            switch (attr) {
                case R.styleable.IndictorView_indicator_icon:
                    //通过自定义属性拿到指示器
                    mIndicator = typedArray.getDrawable(attr);
                    break;
                case R.styleable.IndictorView_indicator_margin:
                    float defaultMargin = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics());
                    mMargin = (int) typedArray.getDimension(attr, defaultMargin);
                    break;
                case R.styleable.IndictorView_indicator_smooth:
...
//    测量View 的大小
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        int desired = getPaddingLeft() + getPaddingRight() +
                mIndicatorSize * mCount + mMargin * (mCount - 1);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        * */
        canvas.restore();
        /*这里又开始计算绘制的位置了*/
        float leftDraw = (mIndicatorSize + mMargin) * (mSelectItem + mOffest);
        /*
    ...
    public void setViewPager(ViewPager viewPager) {
        ...
        PicLoopAdapter pagerAdapter = (PicLoopAdapter) viewPager.getAdapter();
        if (pagerAdapter == null) {
            throw new RuntimeException("请看使用说明");
        }
        mCount = pagerAdapter.getMyCount();
    ...
    public void setOnPageChangeListener(ViewPager.OnPageChangeListener mPageChangeListener) {
        this.mPageChangListener = mPageChangeListener;
    }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mSmooth) {
            mSelectItem = position%mCount;
            mOffest = positionOffset;
            invalidate();
        }
        if (mPageChangListener != null) {
            mPageChangListener.onPageScrolled(position, positionOffset, positionOffsetPixels);
        }
    }
    ...
    public void onPageSelected(int position) {
        ...
        if (mPageChangListener != null) {
            mPageChangListener.onPageSelected(position);
        }
    }
    ...
    public void onPageScrollStateChanged(int state) {
        ...
        if (mPageChangListener != null) {
            mPageChangListener.onPageScrollStateChanged(state);
        }
    }
}